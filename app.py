...
AUDIO_EXTENSIONS = [".mp3", ".wav", ".flac", ".ogg"]   # ← removed .m4a (unsupported by soundfile)
...
def plot_offset_histogram(offsets: list, winner_name: str):
    """Plot histogram of time offsets for the winning song. Highlights the peak bin."""
    if not offsets:
    ...
    display_name = os.path.splitext(winner_name)[0].replace("_", " ")
    ax.set_title(f"Offset Histogram — {display_name}")
...
def plot_votes(ranked, song_index, top_n=10):
    top    = ranked[:top_n]
    labels = [os.path.splitext(song_index[sid])[0].replace("_", " ")
              for sid, _ in top]
...
# ══════════════════════════════════════════════════════════════════════════════
# Shared helper: process one uploaded file object → identification results
# ══════════════════════════════════════════════════════════════════════════════
...
with st.sidebar:
    ...
    if database is None:
        ...
        for sid in sorted(song_index):
            label = os.path.splitext(song_index[sid])[0].replace("_", " ")
            st.markdown(f"🎵 {label}")
...
with tab1:
    ...
    if uploaded is None:
        # FIX: use a flag instead of st.stop() so tab2 still renders
        ...
        if decode_ok:
            ...
            if not ranked:
                ...
                # Display name: clean for UI only
                winner_display = os.path.splitext(winner_name)[0].replace("_", " ")

                st.subheader("⏱️ Offset Histogram")
                offsets = compute_offsets(hashes, winner_id, database)

                if offsets:
                    fig_hist, peak_offset, peak_count = plot_offset_histogram(
                        offsets, winner_name
                    )
                    st.pyplot(fig_hist)
                ...
                if len(ranked) >= 2:
                    ...
                    runner_name               = os.path.splitext(song_index[runner_id])[0].replace("_", " ")
                ...
                st.success(f"### 🎶 {winner_display}")

                r1, r2, r3, r4 = st.columns(4)
                r1.metric("Winner votes",    winner_votes)
                r2.metric("Runner-up",       runner_name, delta=f"−{margin} votes")
                ...
                with st.expander("Full vote table"):
                    ...
                    for rank, (sid, v) in enumerate(ranked, 1):
                        rows.append({
                            "Rank":      rank,
                            "Song":      os.path.splitext(song_index[sid])[0].replace("_", " "),
                            "Votes":     v,
...
with tab2:
    ...
    if not batch_files:
        ...
        for i, f in enumerate(batch_files):
            ...
            try:
                ...
                if batch_ranked:
                    ...
                    pred_raw   = song_index[pred_id]
                    # ── FIX: prediction = filename WITHOUT extension, underscores preserved
                    pred_label = os.path.splitext(pred_raw)[0]
            ...
            batch_results.append({
                "filename":        f.name,
                "prediction":      pred_label,          # raw label for CSV
                "prediction_disp": pred_label.replace("_", " "),  # pretty for table
                "votes":           pred_votes,
        ...
        st.dataframe(
            df_display[["filename", "prediction_disp", "votes", "confidence"]],
            use_container_width=True,
            column_config={
                "filename":        st.column_config.TextColumn("File"),
                "prediction_disp": st.column_config.TextColumn("Prediction"),
        ...
        # ── CSV export (exact rubric format: filename,prediction) ─────────────
        # prediction column = song filename WITHOUT extension, underscores preserved
        ...
        st.download_button(
            ...
            mime="text/csv",
            help="CSV format: filename,prediction  (prediction = song filename without extension)",
        )
        ...
        if len(batch_results) > 1:
            ...
            for val, bar in zip(conf_vals, ax_b.patches):
